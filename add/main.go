// Command add evaluates an "add" DMN model against a kogito-jit (JIT DMN
// executor) service running in Docker.
package main
...
func main() {
	...
	a := flag.Float64("a", 0, "value bound to the DMN input data 'A'")
	b := flag.Float64("b", 0, "value bound to the DMN input data 'B'")
	flag.Parse()
	...
	payload := jitDMNPayload{
		Model:   addDMN,
		Context: map[string]any{"A": *a, "B": *b},
	}
